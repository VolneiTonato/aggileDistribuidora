...
module.exports = class FinanceiroReceitasFormTrocoView extends Backbone.View {
    ...
    reset() {
        this.$el.off('click', '#form-troco-receitas .btn-lancar-troco-historico')
    }
    ...
    overrideEvents() {
        ...
        this.delegateEvents({
            'click #form-troco-receitas .btn-lancar-troco-historico': 'lancarTroco'
        })
    ...
    [emitEventTroco](troco = {}){
        utils.EventUtil.emit('event.financeiro.receita.callFormTrocoInformado', { troco })
    }
    ...
    async lancarTroco(e) {
        ...
        let troco = utils.NumberUtil.cdbl($(e.currentTarget).closest('#form-troco-receitas').find('input[name=receita_troco]').val())
    ...
    async formTroco(troco) {
        ...
        let html = await utils.UnderscoreUtil._template('#template-financeiro-receita-lancamento-troco-form', { troco })
